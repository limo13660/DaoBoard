...
class AuthController extends Controller
{
    ...
    {
        if (!(int)config('daoboard.login_with_mail_link_enable')) {
            abort(404);
        }
        ...
        if (config('daoboard.app_url')) {
            $link = config('daoboard.app_url') . $redirect;
        } else {
            $link = url($redirect);
        }
    ...
    {
        if ((int)config('daoboard.register_limit_by_ip_enable', 0)) {
            $registerCountByIP = Cache::get(CacheKey::get('REGISTER_IP_RATE_LIMIT', $request->ip())) ?? 0;
            if ((int)$registerCountByIP >= (int)config('daoboard.register_limit_count', 3)) {
                abort(500, __('Register frequently, please try again after :minute minute', [
                    'minute' => config('daoboard.register_limit_expire', 60)
                ]));
            }
        }
        ...
        if ((int)config('daoboard.stop_register', 0)) {
            abort(500, __('Registration has closed'));
        }
        if ((int)config('daoboard.invite_force', 0)) {
            if (empty($request->input('invite_code'))) {
                abort(500, __('You must use the invitation code to register'));
            }
        }
        if ((int)config('daoboard.email_verify', 0)) {
        ...
        if ($request->input('invite_code')) {
            ...
            if (!$inviteCode) {
                ...
                if (!(int)config('daoboard.invite_never_expire', 0)) {
                    $inviteCode->status = 1;
                    $inviteCode->save();
                }
            }
        ...
        // try out
        if ((int)config('daoboard.try_out_plan_id', 0)) {
            $plan = Plan::find(config('daoboard.try_out_plan_id'));
            if ($plan) {
                $user->transfer_enable = $plan->transfer_enable * 1073741824;
                $user->device_limit = $plan->device_limit;
                $user->plan_id = $plan->id;
                $user->group_id = $plan->group_id;
                $user->expired_at = time() + (config('daoboard.try_out_hour', 1) * 3600);
        ...
        if ((int)config('daoboard.register_limit_by_ip_enable', 0)) {
            Cache::put(
                ...
                (int)config('daoboard.register_limit_expire', 60) * 60
            );
        }
    ...
    public function login(AuthLogin $request)
    {
        ...
        if ((int)config('daoboard.password_limit_enable', 1)) {
            $passwordErrorCount = (int)Cache::get(CacheKey::get('PASSWORD_ERROR_LIMIT', $email), 0);
            if ($passwordErrorCount >= (int)config('daoboard.password_limit_count', 5)) {
                abort(500, __('There are too many password errors, please try again after :minute minutes.', [
                    'minute' => config('daoboard.password_limit_expire', 60)
                ]));
            }
        }
        ...
        if (!Helper::multiPasswordVerify(
            ...
            if ((int)config('daoboard.password_limit_enable')) {
                Cache::put(
                    CacheKey::get('PASSWORD_ERROR_LIMIT', $email),
                    (int)$passwordErrorCount + 1,
                    60 * (int)config('daoboard.password_limit_expire', 60)
                );
            }
            abort(500, __('Incorrect email or password'));
        }
    ...
    public function token2Login(Request $request)
    {
        if ($request->input('token')) {
            ...
            if (config('daoboard.app_url')) {
                $location = config('daoboard.app_url') . $redirect;
            } else {
