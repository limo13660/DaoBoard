...
class UniProxyController extends Controller
{
    ...
    public function alive(Request $request)
    {
        ...
        foreach ($data as $uid => $ips) {
            ...
            $count = 0;
            if (config('daoboard.device_limit_mode', 0) == 1) {
                $ipmap = [];
    ...
    public function config(Request $request)
    {
        ...
        $response['base_config'] = [
            'push_interval' => (int)config('daoboard.server_push_interval', 60),
            'pull_interval' => (int)config('daoboard.server_pull_interval', 60)
        ];
