...
class DeepbworkController extends Controller
{
    ...
    public function user(Request $request)
    {
        ...
        foreach ($users as $user) {
            $user->v2ray_user = [
                "uuid" => $user->uuid,
                "email" => sprintf("%s@daoboard.user", $user->uuid),
                "alter_id" => 0,
                "level" => 0,
            ];
            unset($user['uuid']);
...
//         Log::info('serverSubmitData:' . $request->input('node_id') . ':' . request()->getContent() ?: json_encode($_POST));
    ...
    private function getV2RayConfig(int $nodeId, int $localPort)
    {
        ...
        $json->log->loglevel = (int)config('daoboard.server_log_enable') ? 'debug' : 'none';
    ...
    {
        $domainRules = array_filter(explode(PHP_EOL, config('daoboard.server_v2ray_domain')));
        $protocolRules = array_filter(explode(PHP_EOL, config('daoboard.server_v2ray_protocol')));
